...
$(document).ready(function() {
  ...
  $('#dataTable').DataTable({
    ...
    "processing": true,
    "ajax": {
        "url": getContextPath()+"/admin/advertisement/listSelect.do",
        "type": "POST",
    ...
    buttons: [
		{
			...
			,filename: '국비의모든것 광고내역'+year+month+date
			,title: '국비의모든것 광고내역'+year+month+date
		...
		},
		{
			...
			,filename: '국비의모든것 광고내역(utf-8이라서 ms엑셀로 바로 열면 글자 깨짐)'+year+month+date
		},
		{
			extend: 'csv'
...
//      let searchTypeVal = target.val();
   ...
   $('#start_date, #end_date').bind('change',function(){
	    if($("#start_date").val() ==''){
	    	...
	    	if(test_date()){	//날짜유효성검사 통과시
		    	...
		    	table.column(4).search(join_date);
  ...
  $(document).on('click', '#dataTable > tbody > tr' , function(e){
	...
	const target = $(e.currentTarget);
	const advertisement_num = target.children(":first").text();
	location.href = getContextPath()+"/admin/advertisement/detail.do?advertisement_num="+advertisement_num;
  });
