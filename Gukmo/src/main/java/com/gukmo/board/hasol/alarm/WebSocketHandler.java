...
public class WebSocketHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
    	...
    	System.out.println("~~~ 웹채팅 확인용 : " + session.getId() + " 이(가)접속했습니다.");
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
    	...
    	if (msg != null) {
    		...
    		if(str != null && str.length == 6) {
    			...
    			String cmd = str[0]; // 알람 구분(댓글, 좋아요, 신고)
    			String boardno = str[1]; // 게시판 번호
    			String boardWriter = str[2]; // 글작성자
    			String commentWriter = str[3]; // 댓글작성자
    			String subject = str[4]; // 게시글 제목
    			String comment = str[5]; // 댓글 
    			// String cnt = str[6];
    			...
    			if("comment".equals(cmd) && boardWriterSession != null) {
    				...
    				TextMessage tmpMsg = new TextMessage("<a href='<%=ctxPath =%>/detail.do?boardNum=" +boardno+ "' > [" +subject+ "] 글에 댓글이 달렸습니다.</a>");
    			...
    			if("like".equals(cmd) && boardWriterSession != null) {
    				...
    				TextMessage tmpMsg = new TextMessage("<a href='<%=ctxPath =%>/detail.do?boardNum=" +boardno+ "'> [" +subject+ "] 글이 좋아요를 받았습니다.</a>");
					boardWriterSession.sendMessage(tmpMsg);
    			}
    			
    			// 게시글이 신고를 받은 경우
    			if("penalty".equals(cmd) && boardWriterSession != null) {
    				if(subject.length() > 13) {
    					subject = subject.substring(0, 13) + "...";
    				}
    					
    				TextMessage tmpMsg = new TextMessage("<a href='<%=ctxPath =%>/detail.do?boardNum=" +boardno+ "' > [" +subject+ "] 글이 신고를 받았습니다.</a>");
					boardWriterSession.sendMessage(tmpMsg);
    			}
    			...
    			// 댓글에 대댓글이 달린 경우
    			if("recomment".equals(cmd) && boardWriterSession != null) {
    				if(comment.length() > 13) {
    					comment = comment.substring(0, 13) + "...";
    				}
    				TextMessage tmpMsg = new TextMessage("<a href='<%=ctxPath =%>/detail.do?boardNum=" +boardno+ "'> [" +comment+ "] 댓글에 댓글이 달렸습니다.</a>");
					commentWriterSession.sendMessage(tmpMsg);
    			}
    			
    			// 댓글이 좋아요를 받은 경우
    			if("commentlike".equals(cmd) && boardWriterSession != null) {
    				if(comment.length() > 13) {
    					comment = comment.substring(0, 13) + "...";
    				}
    				TextMessage tmpMsg = new TextMessage("<a href='<%=ctxPath =%>/detail.do?boardNum=" +boardno+ "' > [" +comment+ "] 댓글이 좋아요를 받았습니다.</a>");
					commentWriterSession.sendMessage(tmpMsg);
    			}
    			
    			// 댓글이 신고를 받은 경우
    			if("commentPenalty".equals(cmd) && boardWriterSession != null) {
    				if(comment.length() > 13) {
    					comment = comment.substring(0, 13) + "...";
    				}
    				TextMessage tmpMsg = new TextMessage("<a href='<%=ctxPath =%>/detail.do?boardNum=" +boardno+ "' > [" +comment+ "] 댓글이 신고를 받았습니다.</a>");
					commentWriterSession.sendMessage(tmpMsg);
    			}
